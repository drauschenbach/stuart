## [unreleased]
### Added
- Support a `stuart.interface.clock` module that can be preloaded with a custom implementation that bind Stuart to a proprietary hardware clock, instead of always depending on LuaSocket for time which may be unavailable in microcontroller environments
