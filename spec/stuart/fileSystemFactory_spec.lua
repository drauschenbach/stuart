...
describe('fileSystemFactory', function()
  ...
  describe('createForOpenPath()', function()
    ...
    it('returns a LocalFileSystem for an absolute path', function()
      local fs, openPath = fileSystemFactory.createForOpenPath('/rom/foo.txt')
      assert.is_true(class.istype(fs, LocalFileSystem))
      assert.equals('/rom', fs:getUri())
      assert.equals('foo.txt', openPath)
    end)
    ...
    it('injects /webhdfs/v1 for a WebHDFS URL that omits it', function()
